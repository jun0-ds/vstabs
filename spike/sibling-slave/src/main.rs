// sibling-slave-spike — vstabs feasibility check #2
//
// Embodies the spawn-and-track ownership model (ADR 2026-04-26):
//   - vstabs spawns its own VS Code instance (no grabbing external windows)
//   - tracks it via hwnd-diff (snapshot before spawn, find new entry after)
//   - container window's geometry → child VS Code's geometry, slaved
//   - container close releases the child (it stays running, no reparent)
//
// This is NOT a reparent. The child VS Code remains a top-level window;
// we only move/resize it from the outside, so IME / GPU / focus are native.
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    GetWindowTextW, IsWindowVisible, SetWindowPos, ShowWindow, HWND_TOP, SWP_FRAMECHANGED,
    SWP_NOACTIVATE, SWP_NOZORDER, SW_RESTORE,
};

// ---- Configure ----
//
// Pick a path you want VS Code to open. Use a path you don't already have open.
...
impl App {
    ...
    fn spawn_vscode(&mut self) {
        ...
        // Snapshot existing VS Code windows so we ignore them forever.
        ...
        // `code` is a .cmd shim, so we route through cmd /C.
        // --new-window forces a fresh hwnd even if VS Code is already running.
            ...
            Ok(child) => {
                println!(
                    "[spike] launched `code --new-window {}` (cmd PID {}). \
                     The actual VS Code hwnd will be a different process — captured by hwnd-diff.",
                    TARGET_PATH,
                    child.id()
                );
    ...
    fn poll_for_child(&mut self) {
        ...
        if start.elapsed() > Duration::from_secs(20) {
            eprintln!("[spike] timeout (20s) waiting for new VS Code window. Aborting capture.");
        ...
        for hwnd in enumerate_vscode_hwnds() {
            if !self.pre_spawn_hwnds.contains(&(hwnd.0 as isize)) {
                ...
                // Force-unmaximize. SetWindowPos is silently ignored on
                // maximized windows — most likely cause of "no movement".
                unsafe {
    ...
    fn layout_child(&mut self) {
        ...
        unsafe {
            ...
            let cts_ok = ClientToScreen(container_hwnd, &mut origin).as_bool();
            if !cts_ok {
            ...
            self.layout_calls += 1;
            // Print only first few + every 20th to avoid console spam.
            if self.layout_calls <= 5 || self.layout_calls % 20 == 0 {
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let attrs = Window::default_attributes()
            .with_title("vstabs sibling-slave spike — close to release child")
    ...
    fn window_event(
        ...
        match event {
            WindowEvent::CloseRequested => {
                println!(
                    "[spike] container closing. Child VS Code stays alive (hwnd={:?}). \
                     No reparent was done, so no detach is needed.",
                    self.child_hwnd
                );
                event_loop.exit();
            }
    ...
    fn about_to_wait(&mut self, _event_loop: &ActiveEventLoop) {
        self.poll_for_child();
        self.layout_child();
        // Poll-based pacing — winit Wait/WaitUntil cadence is unreliable when
        // the container window receives no input events. Sleep here to cap CPU.
